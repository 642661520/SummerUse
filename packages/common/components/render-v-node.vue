<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      return () => {
        if (!props.dynamicVNode) return h('span');
        if (typeof props.dynamicVNode === 'string') return props.dynamicVNode;
        if (isVNode(props.dynamicVNode)) return props.dynamicVNode;
